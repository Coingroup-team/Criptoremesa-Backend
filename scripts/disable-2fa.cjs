...
try {
  ...
  if (app && app.pm2_env) {
    // PM2 flattens env vars into pm2_env directly alongside its own keys
    // Also check pm2_env.env in case they're nested
    pm2Env = app.pm2_env.env || app.pm2_env;
    // If still no PG_DB_SM_USER, try the top-level pm2_env (PM2 merges them)
    if (!pm2Env.PG_DB_SM_USER && app.pm2_env.PG_DB_SM_USER) {
      pm2Env = app.pm2_env;
    }
    console.log("✅ Loaded env from PM2 process 'prod-be-bh:api'");
    // Debug: show available PG keys
    const pgKeys = Object.keys(pm2Env).filter((k) => k.startsWith("PG_DB"));
    console.log("PG keys found:", pgKeys);
  } else {
  ...
  console.log(
    "⚠️  PM2 not available (" + e.message + "), falling back to .env",
  );
  require("dotenv").config({
    path: require("path").resolve(__dirname, "..", ".env"),
  });
...
(async () => {
  try {
    ...
    console.log(
      "Schemas with 'users' table:",
      schemas.rows.map((r) => r.schemaname),
    );
