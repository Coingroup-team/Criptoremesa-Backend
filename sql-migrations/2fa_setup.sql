-- ============================================================
-- 2FA Setup Migration for Bithonor
-- Schema: sec_cust (ms_sixmap_users table)
-- Uses email_user as the unique identifier (NOT username)
-- ============================================================

-- 1. Add 2FA columns to the users table
ALTER TABLE sec_cust.ms_sixmap_users
  ADD COLUMN IF NOT EXISTS two_factor_enabled BOOLEAN DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS two_factor_factor_sid VARCHAR(64);

-- 2. Function: get_2fa_status_by_email
-- Returns the 2FA status for a given email_user
CREATE OR REPLACE FUNCTION sec_cust.get_2fa_status_by_email(
  p_email_user VARCHAR
)
RETURNS TABLE(
  uuid_user UUID,
  two_factor_enabled BOOLEAN,
  two_factor_factor_sid VARCHAR
)
LANGUAGE plpgsql AS $$
BEGIN
  RETURN QUERY
  SELECT
    ...
    u.two_factor_factor_sid
  FROM sec_cust.ms_sixmap_users u
  WHERE u.email_user = p_email_user
  LIMIT 1;
END;
$$;

-- 3. Function: sp_save_pending_2fa_factor
-- Saves a pending factorSid (QR generated but not yet verified)
CREATE OR REPLACE FUNCTION sec_cust.sp_save_pending_2fa_factor(
  p_email_user VARCHAR,
  p_factor_sid VARCHAR
)
RETURNS VOID LANGUAGE plpgsql AS $$
BEGIN
  UPDATE sec_cust.ms_sixmap_users
  SET
    two_factor_factor_sid = p_factor_sid,
...
-- 4. Function: sp_enable_2fa
-- Activates 2FA and saves the definitive factorSid
CREATE OR REPLACE FUNCTION sec_cust.sp_enable_2fa(
  p_email_user VARCHAR,
  p_factor_sid VARCHAR
)
RETURNS VOID LANGUAGE plpgsql AS $$
...
-- 5. Function: sp_disable_2fa
-- Disables 2FA and clears the factorSid
CREATE OR REPLACE FUNCTION sec_cust.sp_disable_2fa(
  p_email_user VARCHAR
)
RETURNS VOID LANGUAGE plpgsql AS $$
BEGIN
...
-- ============================================================
-- 6. Update get_all_users_by_email to include two_factor_enabled
-- The function uses RETURNS TABLE with explicit columns,
-- so the new column must be added manually.
-- ============================================================
...
CREATE OR REPLACE FUNCTION sec_cust.get_all_users_by_email(_email_user character varying)
 RETURNS TABLE(
    ...
    persona_inquiry_id character varying,
    two_factor_enabled boolean  -- 2FA FIELD ADDED
)
 LANGUAGE plpgsql
AS $function$
BEGIN
    ...
    SELECT
        ...
        US.persona_inquiry_id,
        COALESCE(US.two_factor_enabled, FALSE)  -- 2FA FIELD ADDED IN SELECT
