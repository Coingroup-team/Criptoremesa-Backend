...
remittancesService.getRemittances = async (req, res, next) => {
  try {
    ...
    let data = await remittancesPGRepository.getRemittances(
...
remittancesService.limitationsByCodPub = async (req, res, next) => {
  try {
    ...
    let data = await remittancesPGRepository.limitationsByCodPub(
...
remittancesService.webpayRemittance = async (req, res, next) => {
  try {
    ...
    let transbankRes =
      await transbankService.confirmWebpayTransactionNoEndpoint(req.body.token);
    ...
    if (transbankRes && transbankRes.status === 200) {
      ...
      if (remittance.rateValue.wholesale_partner_rate_factor) {
        remittance.totalWholesalePartnerOriginAmount =
          remittance.totalOriginRemittance;
        if (remittance.rateValue.operation === "mul")
          remittance.totalOriginRemittance =
            remittance.totalDestinationRemittance /
            remittance.rateValue.rate_factor;
        else if (remittance.rateValue.operation === "div")
          remittance.totalOriginRemittance =
            remittance.totalDestinationRemittance *
            remittance.rateValue.rate_factor;

        remittance.wholesalePartnerProfit = Math.abs(
          remittance.totalOriginRemittance -
            remittance.totalWholesalePartnerOriginAmount,
        );
        ...
        infoForApi = await remittancesPGRepository.getInfoForRateApi(
          remittance.email_user,
        );
        // se obtienen las tasas de la API

        fullRateFromAPI = await axios.get(
          `https://api.currencyfreaks.com/latest?base=${remittance.countryCurrency.isoCode}&symbols=${infoForApi.origin_currency_iso_code},${infoForApi.wholesale_partner_origin_currency_iso_code},USD&apikey=${env.CURRENCY_FREAKS_API_KEY}`,
        );
      } else {
        ...
        fullRateFromAPI = await axios.get(
          `https://api.currencyfreaks.com/latest?base=${remittance.countryCurrency.isoCode}&symbols=${infoForApi.origin_currency_iso_code},USD&apikey=${env.CURRENCY_FREAKS_API_KEY}`,
        );
      }
      console.log("8 startRemittance");
      // se obtienen las tasas de la moneda local del usuario y en dólares

      let localRateFromAPI =
        fullRateFromAPI.data.rates[infoForApi.origin_currency_iso_code];
      localRateFromAPI = parseFloat(localRateFromAPI);

      let WPRateFromAPI =
        fullRateFromAPI.data.rates[
          infoForApi.wholesale_partner_origin_currency_iso_code
        ];
      WPRateFromAPI = parseFloat(WPRateFromAPI);
      ...
      remittance.totalDollarOriginRemittance = parseFloat(
        remittance.totalOriginRemittance * (dollarRateFromAPI * 0.97),
      ).toFixed(2);
      remittance.totalOriginRemittanceInLocalCurrency = parseFloat(
        remittance.totalOriginRemittance * localRateFromAPI,
      ).toFixed(2);

      logger.silly(
        `remittance.totalDollarOriginRemittance: ${remittance.totalDollarOriginRemittance}`,
      );
      logger.silly(`dollarRateFromAPI: ${dollarRateFromAPI}`);
      logger.silly(`localRateFromAPI: ${localRateFromAPI}`);
      logger.silly(
        `remittance.totalOriginRemittance: ${remittance.totalOriginRemittance}`,
      );
      logger.silly(
        `remittance.totalOriginRemittanceInLocalCurrency: ${remittance.totalOriginRemittanceInLocalCurrency}`,
      );

      if (remittance.rateValue.wholesale_partner_rate_factor) {
        remittance.wholesalePartnerProfitLocalCurrency = parseFloat(
          remittance.wholesalePartnerProfit * WPRateFromAPI,
        ).toFixed(2);
        remittance.wholesalePartnerProfitDollar = parseFloat(
          remittance.wholesalePartnerProfit * dollarRateFromAPI,
        ).toFixed(2);
      }
...
function waitingPreRemittance(id_pre_remittance) {
  const timmy = setTimeout(async () => {
    let resp =
      await remittancesPGRepository.expiredPreRemittance(id_pre_remittance);
    if (resp.email_user) notifyChanges("expired_remittance", resp);
  }, 900000);
...
remittancesService.getPreRemittanceByUser = async (req, res, next) => {
  try {
    ...
    let data = await remittancesPGRepository.getPreRemittanceByUser(
      req.params.email_user,
    );
...
remittancesService.cancelPreRemittance = async (req, res, next) => {
  try {
    ...
    let data = await remittancesPGRepository.cancelPreRemittance(
      req.params.id_pre_remittance,
    );
...
remittancesService.lastRemittances = async (req, res, next) => {
  try {
    ...
    let data = await remittancesPGRepository.lastRemittances(
      req.params.email_user,
      req.query.limit,
      req.query.start_date,
      req.query.end_date,
      req.query.mode,
      req.query.only_wholesale_partner,
    );
...
remittancesService.getInfoByOriginAndDestination = async (req, res, next) => {
  logger.info(
    `[${context}]: Getting remittance info by origin and destination`,
  );
  ObjLog.log(`[${context}]: Getting remittance info by origin and destination`);

  let data;
  const pairInfo =
    req.params.countryIsoCodOrigin + req.params.countryIsoCodDestiny;
  ...
  if (redisInfo) {
    ...
    data = await remittancesPGRepository.getInfoByOriginAndDestination(
      req.params.countryIsoCodOrigin,
      req.params.countryIsoCodDestiny,
    );
